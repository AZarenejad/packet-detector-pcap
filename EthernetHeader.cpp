...
std::string EthernetHeader::get_ethernet_type() const 
{
    return ether_type;
}
std::string EthernetHeader::get_dest_host() const 
{
    return ether_dst_host;
}
std::string EthernetHeader::get_source_host() const 
{
...
void EthernetHeader::set_mac_src_addr()
{
    ...
    {
        ether_src_host += convert_int_to_string((int) ethernet_header ->ether_shost[i]);
    }
}
...
void EthernetHeader::set_mac_dst_addr(){
    ...
    for (int i=0;i<ETH_ALEN;i++)
    {
        ether_dst_host += convert_int_to_string((int) ethernet_header ->ether_dhost[i]);
    }
}
